...
print(f"src_folder:{config['src_folder']}")
print(f"len:{len(sample_list)}")
...
for idx in tqdm(range(len(sample_list))):
    ...
    if config["use_boostmonodepth"] is True:
        run_boostmonodepth(
            sample["ref_img_fi"], config["src_folder"], config["depth_folder"]
        )
    ...
    config["output_h"], config["output_w"] = (
        int(config["output_h"] * frac),
        int(config["output_w"] * frac),
    )
    ...
    if not (config["load_ply"] is True and os.path.exists(mesh_fi)):
        ...
        depth_edge_weight = torch.load(
            config["depth_edge_model_ckpt"], map_location=torch.device(device)
        )
